...
function ModuloBusqueda() {
  ...
  // =========================================
  useEffect(() => {
    const datosGuardados = JSON.parse(localStorage.getItem('reporteMacro')) || [];
    ...
    if (esAdmin) {
      ...
      const filtrados = datosGuardados.filter((item) => {
        ...
        const codigo = String(item['CODIGO DE INSTRUCTOR'] || '')
          .trim()
          .replace(/\s+/g, '');
  ...
  const obtenerAlerta = (estado, fechaFin) => {
    ...
    if (estadoTexto.includes('ejecución')) {
      ...
      if (typeof fechaFin === 'number') {
        fecha = new Date((fechaFin - 25569) * 86400 * 1000);
        fecha.setDate(fecha.getDate() + 1);
      }
  ...
  const cargarArchivo = async (e) => {
    ...
    reader.onload = (evt) => {
      ...
      localStorage.setItem('reporteMacro', JSON.stringify(json));
        ...
        <div className="bg-white rounded-3xl border border-gray-200 p-8 mb-10 shadow-sm">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div className="w-16 h-16 rounded-2xl bg-violet-100 flex items-center justify-center">
              <Upload size={32} className="text-violet-700" />
            </div>
          ...
          <label className="flex flex-col items-center justify-center border-2 border-dashed border-violet-300 rounded-3xl p-12 cursor-pointer hover:bg-violet-50 transition-all">
              ...
              {datos.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-5 font-semibold text-gray-800">
                      {item['Ficha'] || item['NUMERO DE FICHA'] || 'N/A'}
                    </td>

                    <td className="px-6 py-5 text-gray-700">
                      {item['REPORTE DF_53'] || item['ESTADO DE FICHA DF_53'] || 'N/A'}
                    </td>

                    <td className="px-6 py-5 text-gray-700">
                      {item['REPORTE PE_04'] || item['ESTADO DE FICHA PE_04'] || 'N/A'}
                    </td>

                    <td className="px-6 py-5 text-gray-700">
                      {item['APRENDICES ACTIVOS SIN RUTA'] || item['APRENDICES SIN ENRUTAR'] || '0'}
                    </td>

                    <td className="px-6 py-5 text-gray-700 font-medium">
                      {formatearFecha(item['FECHA INICIO'] || item['Fecha Inicio'])}
                    </td>
                    ...
                    <td className="px-6 py-5">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="font-bold text-violet-700">
                          {item['HORAS DE INSTRUCTOR'] || item['HORAS EJECUTADAS'] || 0} Horas
                        </span>
